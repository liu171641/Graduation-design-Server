...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			<ul class="toolbar">
				<li class="click"
					onclick="window.location.href='admin/newslistAdd.jsp'"><span><img
		...
		<table class="tablelist">
			...
			<tbody>
				...
				<tr style=" text-align:center;">
					...
					<td><%=row.get("name").toString().trim()%></td>
					<td><img style="width: 100px;height: 100px;"
						src="<%=row.get("photo").toString().trim()%>" /></td>
					<td><%=row.get("phonenumber").toString().trim()%></td>
					<td><%=row.get("password").toString().trim()%></td>
					<td><%=row.get("sex").toString().trim()%></td>
					<td><%=row.get("age").toString().trim()%></td>
					<td><%=row.get("address").toString().trim()%></td>
					<td><a
						href="admin/newslistUpdate.jsp?id=<%=row.get("id").toString().trim()%>">修改</a>
						<a href="NewsDelServlet?id=<%=row.get("id").toString().trim()%>"
		...
		<div class="pagin">
			<div class="message">
				共<i class="blue"><%=newslist.size()%></i>条记录，当前显示第&nbsp;<i
					class="blue">1&nbsp;</i>页
			</div>
			<ul class="paginList">
