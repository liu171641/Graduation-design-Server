		...
		<table class="tablelist">
			...
			<tbody>
				...
				<tr style=" text-align:center;">
					...
					<td style="width: 250px;"><%=row.get("title").toString().trim()%></td>
					<td><img style="width: 100px;height: 100px;"
						src="<%=row.get("url").toString().trim()%>" /></td>
					<td><%=row.get("category").toString().trim()%></td>
					<td><%=row.get("price").toString().trim()%></td>
					<td><%=row.get("amount").toString().trim()%></td>
					<td><a
						href="admin/CommodityUpdate.jsp?id=<%=row.get("id").toString().trim()%>">修改</a>
						<a href="CommodityDelServlet?id=<%=row.get("id").toString().trim()%>"
